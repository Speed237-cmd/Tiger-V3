...
//bug cases
case "androcrash": case "xscammer" : {
  if (!isPremium) return replygcxeon(mess.prem)
  if (!text) return replygcxeon(`Use ${prefix+command} victim number|amount\nExample ${prefix+command} 237xxxxxxxxxx,5`) 
...
break;
case "ioscrash": case "ban-tech" : case "polar-bug" : {
  if (!isPremium) return replygcxeon(mess.prem)
  if (!text) return replygcxeon(`Use ${prefix+command} victim number|amount\nExample ${prefix+command} 237xxxxxxxxxx,5`) 
  let number = text.split(',')[0];
  let amount = text.split(',')[1] * 5;
...
break;
case "iosfreeze": case " Beta-V1" :case " Beta-V2" : case " Beta-V3" : case " Beta-V4" :case " Beta-V5" :case " Beta-V6" :
  {
	if (!isPremium) return replygcxeon(mess.prem)
    if (!isBot) {
  ...
  break;
  case "gcbug": case "xgc" : case "crashgc" : 
  {
    if (!isPremium) return replygcxeon(mess.prem)
    if (!text) {
  ...
  break;
  case "Mickey-bug" : case "Blast-bug" :{
  if (!isPremium) return replygcxeon(mess.prem)
  if (!text) return replygcxeon(`Use ${prefix+command} victim number|amount\nExample ${prefix+command} 237xxxxxxxxxx,5`) 
  let number = text.split(',')[0];
  let amount = text.split(',')[1] * 5;
...
break;
case "meta-v1" :case "meta-v2" :case "meta-v3" :case "meta-v4" : case "meta-v5" :case "meta-v6" :{
  if (!isPremium) return replygcxeon(mess.prem)
  if (!text) return replygcxeon(`Use ${prefix+command} victim number|amount\nExample ${prefix+command} 237xxxxxxxxxx,5`) 
  let number = text.split(',')[0];
  let amount = text.split(',')[1] * 5;
...
break
            ...
            case '?':
            case 'allmenu':
                let xeonmenuoh = `╰┈➤ 𝙏𝙄𝙂𝙀𝙍-𝙑3-𝘽𝙊𝙏 ═══⊷
┃❃╭──────────────
┃❃│ Prefix : [ ]
┃❃│ User :  ${pushname}
┃❃│ Time : ${xeonytimewisher}
┃❃│ creator : +237651707126
┃❃│ Version : 1.0
┃❃│ Plugins : 250
┃❃│ Ram : 64 GB
                    ...
                    XeonBotInc.sendMessage(m.chat, {
                        ...
                        contextInfo: {
                            externalAdReply: {
                                title: botname,
                                body: ownername,
                                thumbnailUrl: 'https://envs.sh/SlL.jpg', 
                                sourceUrl: link,
                                mediaType: 1,
                                renderLargerThumbnail: true
                            }
